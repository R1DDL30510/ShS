...
public sealed class FullStackVerificationTests
{
    ...
    public async Task FullStackScenario_RunsDetectionAndLogCollection()
    {
        ...
        workerLogger.Entries.Should().Contain(entry =>
            ...
            entry.Message.Contains("Detected service open-webui", StringComparison.OrdinalIgnoreCase));
        ...
        var records = lines
            .Select(line => JsonSerializer.Deserialize<LogRecord>(line) ?? throw new InvalidOperationException("Failed to parse log record."))
            .ToList();
