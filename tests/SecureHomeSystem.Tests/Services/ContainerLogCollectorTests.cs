...
public sealed class ContainerLogCollectorTests
{
    ...
    public async Task RunOnceAsync_WritesNewEntriesAndUpdatesCursor()
    {
        ...
        var records = lines.Select(line => JsonSerializer.Deserialize<LogRecord>(line) ?? throw new InvalidOperationException("Logeintrag konnte nicht geparst werden.")).ToList();
        ...
        records = lines.Select(line => JsonSerializer.Deserialize<LogRecord>(line) ?? throw new InvalidOperationException("Logeintrag konnte im zweiten Durchlauf nicht geparst werden.")).ToList();
