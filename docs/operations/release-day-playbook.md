...
4. **Tests ausführen**
   - `dotnet test` – stellt sicher, dass die Docker-Detection-Parser weiterhin funktionieren.
   - Artefakt: Testrun-Ausgabe in den Release-Notizen ablegen.
5. **Worker überwachen**
...
8. **Präsentation synchronisieren**
...
4. **Risiken & Flags (Slide 14)**
   ...
   - Testabdeckung noch fokussiert auf Docker-Detection (`README.md`, `SecureHomeSystem.Tests`).
...
| 🟡 `testing-gap` | Automatisierte Tests decken derzeit nur die Docker-Detection ab. | In Q&A betonen, dass weitere Bereiche priorisiert werden; Maintainer:innen um zusätzliche Tests bitten. |
