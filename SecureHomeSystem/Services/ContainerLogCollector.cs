...
/// <summary>
/// Sammelt Logs gelabelter Docker-Container ein und speichert sie als JSON-Zeilen
/// unter <c>/logs/services</c>. Ein Cursor pro Service verhindert Duplikate über Neustarts
/// hinweg und ermöglicht leichte Vergleiche in Probeläufen.
/// </summary>
public sealed class ContainerLogCollector : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogInformation("Container-Log-Collector wurde per Konfiguration deaktiviert.");
            return;
        }
        ...
        _logger.LogInformation(
            "Container-Log-Collector gestartet. Intervall {Interval}s, Rückblick {Lookback}min, Service-Verzeichnis {ServicesDirectory}",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Docker-Logs konnten nicht eingesammelt werden.");
            }
    ...
    private async Task CollectLogsAsync(CancellationToken cancellationToken)
    {
        ...
        if (services.Count == 0)
        {
            _logger.LogDebug("Keine gelabelten Services für die Log-Sammlung gefunden.");
            return;
        }
    ...
    private async Task CollectLogsForServiceAsync(DetectedService service, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning(
                    "docker logs meldete den Exitcode {ExitCode} für Service {Service}. stderr: {StdErr}",
                    result.ExitCode,
                    service.Name,
                    string.IsNullOrWhiteSpace(result.StandardError) ? "(leer)" : result.StandardError.Trim());
            }
            else
            {
                ...
                {
                    _logger.LogInformation(
                        "{Count} Logeinträge für {Service} gesammelt (Container {Container}).",
                ...
                {
                    _logger.LogDebug("Keine neuen Logeinträge für {Service}.", service.Name);
                }
            }
        }
        ...
        {
            _logger.LogError(ex, "Logs für Service {Service} konnten nicht gesammelt werden.", service.Name);
        }
        finally
        {
    ...
    private void EnforceLogRetention(DetectedService service)
    {
        ...
        {
            _logger.LogWarning(ex, "Log-Aufbewahrung konnte für {Service} nicht durchgesetzt werden.", service.Name);
        }
    }
    ...
    private void RotateIfNecessary(DetectedService service, string logPath, DateTimeOffset now)
    {
        ...
        {
            reasons.Add($"Größe {fileInfo.Length}/{_rotationOptions.MaxFileSizeBytes} Bytes");
        }
        ...
        if (_rotationOptions.MaxFileAgeDays > 0)
        {
            ...
            if (age >= TimeSpan.FromDays(_rotationOptions.MaxFileAgeDays))
            {
                ...
                reasons.Add($"Alter {formattedAge}d/{_rotationOptions.MaxFileAgeDays}d");
            }
        }
        ...
        _logger.LogInformation(
            "Servicelog für {Service} rotiert. Archiviert als {Archive} ({Reason}).",
            service.Name,
            archiveName,
            string.Join(", ", reasons));
    }
            ...
            foreach (var archive in archives.ToList())
            {
                ...
                if (lastWrite < cutoff)
                {
                    try
                    {
                        ...
                        _logger.LogDebug(
                            "Abgelaufenes Servicelog-Archiv {Archive} für {Service} gelöscht.",
                            archive.Name,
                            service.Name);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(
                            ex,
                            "Abgelaufenes Log-Archiv {Archive} für {Service} konnte nicht gelöscht werden.",
                            archive.FullName,
                            service.Name);
                    }
                }
        ...
        if (hasCountLimit && archives.Count > _rotationOptions.MaxArchiveFiles)
        {
                         ...
                         .ToList())
            {
                try
                {
                    archive.Delete();
                    _logger.LogDebug(
                        "Überzähliges Servicelog-Archiv {Archive} für {Service} gelöscht.",
                        archive.Name,
                        service.Name);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(
                        ex,
                        "Überzähliges Log-Archiv {Archive} für {Service} konnte nicht gelöscht werden.",
                        archive.FullName,
                        service.Name);
                }
            }
